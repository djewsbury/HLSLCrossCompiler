	...
	uint32_t typeFlag, Instruction* psNextInst)
{
	...
	if (destElemCount > 1)
	{
		...
		AddAssignToDest(psContext, &psInst->asOperands[0], floatResult ? SVT_FLOAT : SVT_UINT, destElemCount, &needsParenthesis);
...
static void WriteSamplerExpression(HLSLCrossCompilerContext* psContext, uint32_t textureRegisterNumber, uint32_t samplerRegisterNumber, const int bZCompare)
{
    bstring glsl = *psContext->currentGLSLString;
    ...
    if (useCombinedTextureSamplers) {
        if (samplerRegisterNumber != ~0) {
            bconcat(glsl, TextureSamplerName(&psContext->psShader->sInfo, textureRegisterNumber, samplerRegisterNumber, bZCompare));
        } else {
            ResourceName(glsl, psContext, RGROUP_TEXTURE, textureRegisterNumber, bZCompare);
        }
    } else if (HaveSeparateTexturesAndSamplers(psContext->psShader->eTargetLanguage, psContext->psShader->extensions)) {
        char* combinedSamplerType = GetSamplerType(psContext, psContext->psShader->aeResourceDims[textureRegisterNumber], textureRegisterNumber);
        bcatcstr(glsl, combinedSamplerType);
        if (bZCompare)
            bcatcstr(glsl, "Shadow");
        bcatcstr(glsl, "(");
        ResourceName(glsl, psContext, RGROUP_TEXTURE, textureRegisterNumber, 0);
        bcatcstr(glsl, ", ");
        if (samplerRegisterNumber != ~0) {
            ResourceName(glsl, psContext, RGROUP_SAMPLER, samplerRegisterNumber, 0);
        } else {
            // These cases are generated by methods such as "Load" in HLSL. Load takes no
            // sampler in HLSL, but GLSL requires some sampler object (see the description of "GL_KHR_vulkan_glsl")
            // We must provide a dummy sampler to get valid GLSL output.
            bcatcstr(glsl, "hlslcc_DummySampler");
        }
        bcatcstr(glsl, ")");
    } else {
		ResourceName(glsl, psContext, RGROUP_TEXTURE, textureRegisterNumber, bZCompare);
	...
	bstring glsl)
{
	...
	{
		WriteSamplerExpression(psContext, psInst->asOperands[2].ui32RegisterNumber, ~0, 0);
		bcatcstr(glsl, ", ");
...
void GetResInfoData(HLSLCrossCompilerContext* psContext, Instruction* psInst, int index, int destElem)
{
	...
	if (index < 3)
	{
		...
		else
		{
            ...
            // This will expand to something like sampler2d(textureName, hlslcc_DummySampler) when using 
            // GL_KHR_vulkan_glsl.
            if (!isUAV) {
	...
	uint32_t ui32Flags)
{
	...
	case RESOURCE_DIMENSION_TEXTURECUBEARRAY:
	{
		...
		if (ui32Flags & TEXSMP_FLAG_DEPTHCOMPARE)
		{
            ...
            WriteSamplerExpression(psContext, psInst->asOperands[2].ui32RegisterNumber, psInst->asOperands[3].ui32RegisterNumber, (ui32Flags & TEXSMP_FLAG_DEPTHCOMPARE) ? 1 : 0);
			bcatcstr(glsl, ",");
	...
	if (ui32Flags & TEXSMP_FLAG_DEPTHCOMPARE)
	{
		...
		SHADER_VARIABLE_TYPE dataType = SVT_FLOAT; // TODO!!
		AddIndentation(psContext);
		AddAssignToDest(psContext, &psInst->asOperands[0], dataType, GetNumSwizzleElements(&psInst->asOperands[2]), &numParenthesis);
		if (ui32Flags & (TEXSMP_FLAG_LOD | TEXSMP_FLAG_FIRSTLOD))
		{
        ...
        WriteSamplerExpression(psContext, psInst->asOperands[2].ui32RegisterNumber, psInst->asOperands[3].ui32RegisterNumber, 1);
	...
	else
	{
        ...
        WriteSamplerExpression(psContext, psInst->asOperands[2].ui32RegisterNumber, psInst->asOperands[3].ui32RegisterNumber, 0);
		bcatcstr(glsl, ", ");
		TranslateTexCoord(psContext, eResDim, &psInst->asOperands[1]);
...
void TranslateInstruction(HLSLCrossCompilerContext* psContext, Instruction* psInst, Instruction* psNextInst)
{
	...
	case OPCODE_GATHER4:
	{
		...
		AddAssignToDest(psContext, &psInst->asOperands[0], SVT_FLOAT, GetNumSwizzleElements(&psInst->asOperands[2]), &numParenthesis);
	...
	case OPCODE_GATHER4_PO_C:
	{
        ...
        WriteSamplerExpression(psContext, psInst->asOperands[3].ui32RegisterNumber, psInst->asOperands[4].ui32RegisterNumber, 1);
		bcatcstr(glsl, ", ");
	...
	case OPCODE_GATHER4_PO:
	{
		...
		AddAssignToDest(psContext, &psInst->asOperands[0], SVT_FLOAT, GetNumSwizzleElements(&psInst->asOperands[2]), &numParenthesis);
		bcatcstr(glsl, "textureGatherOffset(");
        WriteSamplerExpression(psContext, psInst->asOperands[3].ui32RegisterNumber, psInst->asOperands[4].ui32RegisterNumber, 0);
		bcatcstr(glsl, ", ");
        TranslateTexCoord(psContext, eResDim, &psInst->asOperands[1]);
